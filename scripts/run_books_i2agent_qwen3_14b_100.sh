...
# i²Agent includes an ordered feedback-sampling step, so it intentionally
# uses the serial driver (the default --workers=1).
python -m iagent_reproduction.run_official_protocol \
  ...
  --limit 100 \
  --output outputs/official_protocol/books_dynamic_qwen3_14b_fp16_100.jsonl
